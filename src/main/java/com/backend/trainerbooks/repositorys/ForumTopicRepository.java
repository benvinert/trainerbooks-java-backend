...
public interface ForumTopicRepository extends JpaRepository<ForumTopicDAO,Long> {
    List<ForumTopicDAO> findAllByCategoryIdOrderByLastPostCreatedDateDesc(Long categoryId, Pageable pageable);

    @Transactional
    @Query(value = "SELECT t from forum_topics t where t.categoryId = :categoryId AND t.tags LIKE :tag")
    List<ForumTopicDAO> findAllTopicsByCategoryIdAndTagOrderByLastPostCreatedDateDesc(Long categoryId, String tag,Pageable pageable);
}
